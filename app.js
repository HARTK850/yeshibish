...
async function init() {
    ...
    if (state.apiKey && state.booksLoaded === false) {
        updateLoadingStatus('בחר תיקייה עם ספרי קודש...');
    }
    
    // Check if we have a folder handle saved
    const savedFolderHandle = localStorage.getItem('folder_handle');
    if (savedFolderHandle) {
        try {
            const handles = JSON.parse(savedFolderHandle);
            // Try to restore the folder handle
            // Note: This is simplified - actual restoration would need more complex logic
        } catch (e) {
            // Handle might be expired, we'll ask for it again
        }
    }
...
// === Folder Scanner Functions ===
async function selectBooksFolder() {
    try {
        const folderHandle = await window.showDirectoryPicker({
            mode: 'read'
        });
        
        state.selectedFolderHandle = folderHandle;
        updateLoadingStatus('סורק תיקייה...');
        
        const files = await scanFolderRecursive(folderHandle);
        
        if (files.length === 0) {
            alert('לא נמצאו קבצי טקסט בתיקייה');
            updateLoadingStatus('בחר תיקייה עם ספרי קודש');
            return;
        }
        
        updateLoadingStatus(`נמצאו ${files.length} קבצים. טוען...`);
        ...
        for (let i = 0; i < files.length; i++) {
            try {
                ...
                updateLoadingStatus(`טוען ספרי קודש... (${i + 1}/${files.length})`);
            } catch (fileError) {
                console.warn(`לא ניתן לטעון קובץ: ${files[i].path}`, fileError);
            }
        }
        ...
        if (allContent.length > 0) {
            ...
            updateLoadingStatus(`נטענו ${allContent.length} קבצים בהצלחה!`);
            
            // Wait a moment for the user to see the status
            await new Promise(resolve => setTimeout(resolve, 1000));
            showChatScreen();
        } else {
            alert('לא ניתן היה לטעון קבצים מהתיקייה');
            updateLoadingStatus('בחר תיקייה עם ספרי קודש');
        }
    } catch (error) {
...
async function scanFolderRecursive(folderHandle, basePath = '') {
    const files = [];
    ...
    try {
        for await (const entry of folderHandle.entries()) {
            ...
            if (entryHandle.kind === 'file' && (name.endsWith('.txt') || name.endsWith('.text'))) {
                files.push({
                    path: currentPath,
                    file: entryHandle
                });
                ...
                const subFiles = await scanFolderRecursive(entryHandle, currentPath);
                files.push(...subFiles);
            }
        }
    } catch (error) {
...
function saveApiKey() {
    ...
    // If books aren't loaded yet, show folder picker
