...
// === Initialize ===
async function init() {
    updateLoadingStatus('טוען ספרי קודש...');
    ...
    try {
        ...
        updateLoadingStatus('לא נמצא קובץ ספרי קודש. ממשיך בלי ספרים...');
    }
...
function updateBooksStatus() {
    ...
    if (state.booksLoaded) {
        ...
        text.textContent = `ספרי הקודש נטענו (${sizeMB} MB)`;
    } else {
        indicator.classList.add('error');
        text.textContent = 'לא נמצא קובץ ספרי קודש (combined_output.txt)';
    }
}
